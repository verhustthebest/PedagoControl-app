...
const parental_invoice_dto_1 = require("../dto/parental-invoice.dto");
...
async function generateInvoice(request, response) {
    ...
    try {
        ...
        return response.status(201).json({ invoice: (0, parental_invoice_dto_1.parentalInvoiceDetailDto)(invoice) });
    }
    catch (error) {
...
async function indexInvoices(request, response) {
    try {
        const result = await (0, parental_billing_service_1.listParentalInvoices)(parameter(request, 'schoolId'), {
            page: query(request.query.page), limit: query(request.query.limit), status: query(request.query.status),
        });
        return response.json({ invoices: result.invoices.map(parental_invoice_dto_1.parentalInvoiceListDto), pagination: result.pagination });
    }
    catch (error) {
...
async function showInvoice(request, response) {
    try {
        return response.json({
            invoice: (0, parental_invoice_dto_1.parentalInvoiceDetailDto)(await (0, parental_billing_service_1.getParentalInvoice)(parameter(request, 'schoolId'), parameter(request, 'invoiceId'))),
        });
    }
    catch (error) {
...
async function createPayment(request, response) {
    ...
    try {
        ...
        return response.status(201).json({ payment: (0, parental_invoice_dto_1.parentalPaymentDto)(payment) });
    }
    catch (error) {
