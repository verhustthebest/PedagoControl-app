...
function structuredLog(level, event) {
...
function securityLog(request, action, result, details = {}) {
...
function requestContext(request, response, next) {
    ...
    const requestId = incoming && REQUEST_ID.test(incoming) ? incoming : (0, crypto_1.randomUUID)();
    ...
    response.json = ((body) => {
        if (response.statusCode >= 400 && body && typeof body === 'object' && !Array.isArray(body)) {
            if (response.statusCode >= 500)
                return originalJson({ message: 'Internal server error', request_id: requestId });
            return originalJson({ ...body, request_id: requestId });
        }
