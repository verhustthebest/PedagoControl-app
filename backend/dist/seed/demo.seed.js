...
async function upsertRole(seed) {
    return client_1.default.roles.upsert({
...
async function ensureChapter(params) {
    return client_1.default.program_chapters.upsert({
        where: {
...
async function ensureSubChapter(params) {
    return client_1.default.program_sub_chapters.upsert({
        where: {
...
async function findOrCreateDistribution(params) {
    const existing = await client_1.default.program_distribution.findFirst({
        where: {
            annual_program_id: params.annualProgramId,
            program_chapter_id: params.chapterId,
    ...
    if (existing) {
        return client_1.default.program_distribution.update({
    ...
    return client_1.default.program_distribution.create({
        data: {
            annual_program_id: params.annualProgramId,
            program_period_id: params.periodId,
            program_chapter_id: params.chapterId,
...
async function findOrCreateReport(params) {
    const existing = await client_1.default.lesson_sessions.findFirst({
        where: {
    ...
    const report = existing
        ? await client_1.default.lesson_sessions.update({
            where: { id: existing.id },
            data: {
        ...
        : await client_1.default.lesson_sessions.create({
            data: {
                program_distribution_id: params.distributionId,
                teacher_assignment_id: params.assignmentId,
                teacher_user_id: params.teacherId,
    ...
    if (params.status !== 'submitted') {
        const validation = await client_1.default.lesson_validations.findFirst({
            where: {
        ...
        if (validation) {
            await client_1.default.lesson_validations.update({
            ...
            await client_1.default.lesson_validations.create({
                data: {
                    lesson_session_id: report.id,
                    prefect_user_id: params.prefectId,
                    decision: params.status,
        ...
        if (params.observation) {
            const comment = await client_1.default.lesson_comments.findFirst({
                where: {
                    lesson_session_id: report.id,
            ...
            if (comment) {
                await client_1.default.lesson_comments.update({
                    where: { id: comment.id },
                    data: { comment_text: params.observation },
                });
            }
            else {
                await client_1.default.lesson_comments.create({
                    data: {
                        lesson_session_id: report.id,
                        user_id: params.prefectId,
                        comment_type: 'observation',
...
async function ensureNotification(params) {
    const existing = await client_1.default.notifications.findFirst({
        where: {
    ...
    if (existing) {
        return client_1.default.notifications.update({
            where: { id: existing.id },
            data: {
    ...
    return client_1.default.notifications.create({
        data: {
            recipient_user_id: params.recipientId,
            sender_user_id: params.senderId,
            title: params.title,
...
async function ensureActivityLog(params) {
    const existing = await client_1.default.activity_logs.findFirst({
        where: {
    ...
    if (existing) {
        return client_1.default.activity_logs.update({
            where: { id: existing.id },
            data: {
                title: params.title,
                description: params.description,
    ...
    return client_1.default.activity_logs.create({
        data: {
            school_id: params.schoolId,
            user_id: params.userId,
            activity_type: params.type,
...
async function main() {
    ...
    const school = await client_1.default.schools.upsert({
    ...
    const educationLevel = await client_1.default.education_levels.upsert({
    ...
    const educationOption = await client_1.default.education_options.upsert({
    ...
    const subject = await client_1.default.subjects.upsert({
    ...
    const academicYear = await client_1.default.academic_years.upsert({
        where: {
            school_id_name: {
                school_id: school.id,
    ...
    const schoolClass = await client_1.default.school_classes.upsert({
        where: {
    ...
    const academicYearClass = await client_1.default.academic_year_classes.upsert({
        where: {
    ...
    const academicYearSubject = await client_1.default.academic_year_subjects.upsert({
        where: {
    ...
    const annualProgram = await client_1.default.annual_programs.upsert({
        where: {
            id: (await client_1.default.annual_programs.findFirst({
    ...
    const period = await client_1.default.program_periods.upsert({
        where: {
            annual_program_id_order_index: {
    ...
    const assignment = await client_1.default.teacher_assignments.upsert({
        where: {
            id: (await client_1.default.teacher_assignments.findFirst({
                where: {
                    academic_year_subject_id: academicYearSubject.id,
