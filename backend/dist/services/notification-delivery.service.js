...
const list = (value, normalizer) => new Set((value || '').split(',').map(item => normalizer(item.trim())).filter(Boolean));
...
async function sendMailtrap(message, environment, fetcher) { const token = environment.MAILTRAP_API_TOKEN, inbox = environment.MAILTRAP_INBOX_ID, from = environment.NOTIFICATION_EMAIL_FROM; if (!token || !inbox || !from)
    return { status: 'PREPARED', channel: 'email', provider: 'mailtrap', reason: 'provider_not_configured' }; const response = await fetcher(`https://sandbox.api.mailtrap.io/api/send/${encodeURIComponent(inbox)}`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: { email: from, name: environment.NOTIFICATION_EMAIL_FROM_NAME || 'PEDAGO CONTROL' }, to: [{ email: message.to }], subject: message.subject || 'PEDAGO CONTROL', text: message.text }) }); if (!response.ok)
    return { status: 'FAILED', channel: 'email', provider: 'mailtrap', reason: 'provider_rejected' }; return { status: 'SENT', channel: 'email', provider: 'mailtrap', provider_message_id: await acceptedId(response) }; }
/** Resend n'est considéré SENT qu'après une réponse HTTP d'acceptation du fournisseur. */
async function sendResend(message, environment, fetcher) { const key = environment.RESEND_API_KEY, from = environment.NOTIFICATION_EMAIL_FROM; if (!key || !from)
    return { status: 'PREPARED', channel: 'email', provider: 'resend', reason: 'provider_not_configured' }; const response = await fetcher('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [message.to], subject: message.subject || 'PEDAGO CONTROL', text: message.text }) }); if (!response.ok)
    return { status: 'FAILED', channel: 'email', provider: 'resend', reason: 'provider_rejected' }; return { status: 'SENT', channel: 'email', provider: 'resend', provider_message_id: await acceptedId(response) }; }
/** Infobip reçoit uniquement du texte et un numéro normalisé ; la clé n'est jamais retournée. */
async function sendInfobip(message, environment, fetcher) { const key = environment.INFOBIP_API_KEY, base = environment.INFOBIP_BASE_URL, from = environment.INFOBIP_SMS_FROM; if (!key || !base || !from)
    return { status: 'PREPARED', channel: 'sms', provider: 'infobip', reason: 'provider_not_configured' }; const response = await fetcher(`${base.replace(/\/$/, '')}/sms/2/text/advanced`, { method: 'POST', headers: { Authorization: `App ${key}`, 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ messages: [{ from, destinations: [{ to: normalizePhone(message.to) }], text: message.text }] }) }); if (!response.ok)
