...
exports.listSubscriptionCatalog = void 0;
exports.saveSchoolDraft = saveSchoolDraft;
exports.finalizeSchool = finalizeSchool;
...
async function saveSchoolDraft(userId, input) {
    ...
    if (input.draft_id) {
        const existing = await client_1.default.school_creation_drafts.findFirst({ where: { public_id: input.draft_id, created_by_user_id: BigInt(userId), status: 'draft' } });
        if (!existing)
            return null;
        return client_1.default.school_creation_drafts.update({ where: { id: existing.id }, data: { payload, updated_at: new Date() }, select: { public_id: true, status: true, updated_at: true } });
    }
    return client_1.default.school_creation_drafts.create({ data: { created_by_user_id: BigInt(userId), payload }, select: { public_id: true, status: true, updated_at: true } });
}
const listSubscriptionCatalog = () => client_1.default.subscriptions.findMany({
...
async function finalizeSchool(userId, input) {
    ...
    if (draft.status === 'completed' && draft.created_school_public_id)
        return { kind: 'created', public_id: draft.created_school_public_id, repeated: true };
    const claimed = await client_1.default.school_creation_drafts.updateMany({ where: { id: draft.id, status: 'draft' }, data: { status: 'creating', updated_at: new Date() } });
    if (claimed.count !== 1)
        return { kind: 'conflict' };
    try {
        const passwordHash = await bcrypt_1.default.hash(input.account.password, 12);
        const result = await client_1.default.$transaction(async (transaction) => {
            ...
            const school = await transaction.schools.create({ data: {
                    code: `PED-${new Date().getUTCFullYear()}-${(0, node_crypto_1.randomUUID)().slice(0, 8).toUpperCase()}`, name: input.school.name,
        ...
        return { kind: 'created', public_id: result, repeated: false };
    }
    catch (error) {
