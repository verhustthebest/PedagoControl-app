...
exports.listSchoolStaff = listSchoolStaff;
exports.updateSchoolStaff = updateSchoolStaff;
...
const action_token_service_2 = require("./action-token.service");
...
const opaque = (schoolId, kind, id) => { const hash = (0, crypto_1.createHash)('sha256').update(`${kind}:${schoolId}:${id}`).digest('hex'); return `${hash.slice(0, 8)}-${hash.slice(8, 12)}-4${hash.slice(13, 16)}-a${hash.slice(17, 20)}-${hash.slice(20, 32)}`; };
...
async function createSchoolStaff(schoolId, input, actorId, requestId) {
    if (input.phone)
        input.phone = (0, phone_identity_1.normalizeDrcPhone)(input.phone);
    const passwordHash = await bcrypt_1.default.hash((0, crypto_1.randomBytes)(32).toString('base64url'), 12);
    const staff = await client_1.default.$transaction(async (transaction) => {
        if (input.phone)
        ...
        if (emailOwner)
            throw new SchoolStaffError('Cet e-mail est déjà associé à un compte utilisateur.', 409);
    ...
    const internal = await client_1.default.users.findUniqueOrThrow({ where: { public_id: staff.public_id }, select: { id: true } });
    const invitation = await (0, action_token_service_1.issueActionToken)('invitation', { userId: internal.id.toString() });
    const link = `${(process.env.FRONTEND_URLS || '').split(',')[0].replace(/\/$/, '')}/invitation?token=${encodeURIComponent(invitation.token)}`;
    const email = await (0, notification_delivery_service_1.deliverNotification)({ channel: 'email', to: input.email, subject: 'Invitation PEDAGO CONTROL', text: `Votre compte a été créé. Activez-le via ce lien sécurisé : ${link}` });
    const sms = input.phone ? await (0, notification_delivery_service_1.deliverNotification)({ channel: 'sms', to: input.phone, text: `PEDAGO CONTROL : une invitation sécurisée a été envoyée pour activer votre compte.` }) : null;
    const occurredAt = new Date();
    const deliverySummary = (delivery) => delivery ? `${delivery.channel}:${delivery.provider}:${delivery.status}` : 'sms:non_demande';
    await client_1.default.$transaction([
        client_1.default.notifications.create({ data: {
        ...
        client_1.default.activity_logs.create({ data: {
...
async function acceptStaffInvitation(token, password) {
    const action = await (0, action_token_service_2.consumeActionToken)(token, 'invitation');
    if (!action.user_id)
        throw new SchoolStaffError('Invitation invalide ou expirée.', 400);
    const passwordHash = await bcrypt_1.default.hash(password, 12);
    const user = await client_1.default.users.update({
    ...
    const school = await client_1.default.users.findUnique({ where: { id: action.user_id }, select: { school_id: true } });
    if (school?.school_id)
        await client_1.default.activity_logs.create({ data: {
...
async function listSchoolStaff(schoolId, input) {
    const where = { school_id: schoolId, ...(input.role ? { user_roles: { some: { roles: { name: input.role } } } } : {}), ...(input.search ? { OR: [{ first_name: { contains: input.search, mode: 'insensitive' } }, { last_name: { contains: input.search, mode: 'insensitive' } }, { email: { contains: input.search, mode: 'insensitive' } }] } : {}) };
    const [items, total] = await client_1.default.$transaction([
...
async function updateSchoolStaff(schoolId, publicId, data) {
    const current = await client_1.default.users.findFirst({ where: { school_id: schoolId, public_id: publicId }, select: { id: true, first_name: true, last_name: true } });
    if (!current)
        throw new SchoolStaffError('Ressource introuvable.', 404);
    if (data.phone)
        await (0, phone_identity_1.assertPhoneAvailable)({ phone: String(data.phone), first_name: String(data.first_name || current.first_name), last_name: String(data.last_name || current.last_name), schoolId });
    return client_1.default.users.update({ where: { id: current.id }, data: { ...data, ...(data.birth_date ? { birth_date: new Date(`${data.birth_date}T00:00:00.000Z`) } : {}), ...(data.phone ? { phone: (0, phone_identity_1.normalizeDrcPhone)(String(data.phone)) } : {}) }, select: { public_id: true, first_name: true, last_name: true, email: true, phone: true, birth_date: true, is_active: true } });
}
async function assignTeacher(schoolId, teacherPublicId, classSubjectIds, assignedBy) {
    const teacher = await client_1.default.users.findFirst({ where: { school_id: schoolId, public_id: teacherPublicId, user_roles: { some: { roles: { name: 'ENSEIGNANT' } } } }, select: { id: true } });
    if (!teacher)
        throw new SchoolStaffError('Ressource introuvable.', 404);
    const subjects = await client_1.default.academic_year_subjects.findMany({ where: { public_id: { in: classSubjectIds }, academic_year_classes: { academic_years: { school_id: schoolId } } }, select: { id: true } });
    if (subjects.length !== new Set(classSubjectIds).size)
        throw new SchoolStaffError('Affectation invalide.', 400);
    await client_1.default.$transaction(async (tx) => { for (const subject of subjects) {
        const existing = await tx.teacher_assignments.findFirst({ where: { academic_year_subject_id: subject.id, teacher_user_id: teacher.id }, select: { id: true } });
        if (existing)
            await tx.teacher_assignments.update({ where: { id: existing.id }, data: { status: 'active', end_date: null, assigned_by_user_id: BigInt(assignedBy) } });
        else
            await tx.teacher_assignments.create({ data: { academic_year_subject_id: subject.id, teacher_user_id: teacher.id, assigned_by_user_id: BigInt(assignedBy), start_date: new Date(), status: 'active' } });
    } });
